...
function renderSourceManager(){
 ...
 if(stats){const activeSources=sources.filter(s=>s.active).length;const verified=sources.filter(s=>s.verification==='verified'&&s.germanyEligibility==='yes').length;const due=sources.filter(s=>sourceReviewState(s).due).length;const linked=contests.filter(i=>sourceForContest(i)).length;stats.innerHTML=`<div><strong>${sources.length}</strong><span>Quellen gesamt</span></div><div><strong>${verified}</strong><span>DE verifiziert</span></div><div><strong>${due}</strong><span>jetzt prüfen</span></div><div><strong>${linked}/${contests.length}</strong><span>Gewinnspiele verknüpft</span></div>`;renderSourceCoverage()}
 box.innerHTML=rows.length?rows.map(s=>{const c=counts.get(s.id)||{total:0,active:0,top:0};const r=review(s);const home=sourceHomepage(s);const priority=sourcePriority(s,c),health=sourceHealthLabel(s);return `<article class="source-card ${r.due?'source-due':''} ${!s.active?'source-inactive':''}"><div class="source-card-head"><div><strong>${esc(s.name)}</strong><span>${esc(s.domain||s.country)}</span></div><b class="source-priority">${priority}</b></div><div class="source-card-meta"><span>${esc(s.type)}</span><span>${esc(s.country)}</span><span>Qualität ${s.quality}/5</span><span class="source-health ${health.key}">${esc(health.label)}</span><span class="review-chip ${r.key}">${esc(r.label)}</span>${s.local?'<span>Lokal</span>':''}</div><p>${esc(s.notes||'Keine Notiz')}</p><div class="source-card-foot"><span>${c.active} aktiv · ${c.total} gesamt${c.top?` · ${c.top} Top`:''}</span><small>${esc(sourceAutomationLabel(s.automation))} · Erfolg ${s.successfulChecks}/${s.successfulChecks+s.emptyChecks||0}${s.lastChecked?` · ${esc(s.lastChecked)}`:''}</small></div><div class="source-card-actions">${home?`<a href="${esc(home)}" target="_blank" rel="noopener">Website ↗</a>`:''}<button type="button" data-source-found="${esc(s.id)}">＋ Treffer</button><button type="button" data-source-empty="${esc(s.id)}">0 Leer</button><button type="button" data-source-edit="${esc(s.id)}">Bearbeiten</button><button type="button" data-source-toggle="${esc(s.id)}">${s.active?'Pausieren':'Aktivieren'}</button></div></article>`}).join(''):empty('Keine Quellen passen zu den Filtern.');
...
function visibleSources(){
 const q=sourceFilters.search.trim().toLowerCase();return sources.filter(s=>(!q||[s.name,s.domain,s.notes,...s.categories].join(' ').toLowerCase().includes(q))&&(!sourceFilters.country||s.country===sourceFilters.country)&&(!sourceFilters.type||s.type===sourceFilters.type)&&(!sourceFilters.automation||s.automation===sourceFilters.automation)&&(!sourceFilters.review||(sourceFilters.review==='due'?sourceReviewState(s).due:sourceReviewState(s).key===sourceFilters.review)))
}
...
function setupSourceManager(){
 ...
 $('#sourceOverview')?.addEventListener('click',e=>{const found=e.target.closest('[data-source-found]'),emptyBtn=e.target.closest('[data-source-empty]'),edit=e.target.closest('[data-source-edit]'),toggle=e.target.closest('[data-source-toggle]');if(found)recordSourceResult(found.dataset.sourceFound,true);else if(emptyBtn)recordSourceResult(emptyBtn.dataset.sourceEmpty,false);else if(edit){const s=sourceById(edit.dataset.sourceEdit);if(s)openSourceDialog(s)}else if(toggle){const s=sourceById(toggle.dataset.sourceToggle);if(s){persistSource({...s,active:!s.active},s.id);toast(`${s.name} ${s.active?'pausiert':'aktiviert'}`)}}});
}
