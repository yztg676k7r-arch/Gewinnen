...
function stateFor(id){user.items[id]??={favorite:false,done:false,won:false};return user.items[id]}
...
function scored(){return allActive().map(i=>({...i,...scoreContest(i)}))}
...
function matches(i,f){
 if(f==='all')return true;if(f==='recommended')return recommended(i);if(f==='new')return i.new;
 if(f==='top')return i.score>=80;if(f==='secret')return secret(i);if(f==='ending')return daysLeft(i)<=7;
 if(f==='international')return i.international;return i.category===f
}
...
window.toggleFavorite=toggleFavorite;window.toggleDone=toggleDone;window.registerClick=registerClick;

function label(score){return score>=88?'Unbedingt mitmachen':score>=75?'Sehr empfehlenswert':score>=62?'Gute Chance':'Solide Aktion'}
function badges(i){const left=daysLeft(i);return `<div class="badges"><span class="badge">${esc(i.category)}</span><span class="badge score">${i.score}/100</span>${i.score>=80?'<span class="badge score">Top-Chance</span>':''}${secret(i)?'<span class="badge secret">Geheimtipp</span>':''}${left<=3?`<span class="badge hot">Noch ${left} Tag${left===1?'':'e'}</span>`:''}${i.international?'<span class="badge intl">International</span>':''}</div>`}
...
function mini(i){const s=stateFor(i.id);return `<article class="mini-card"><div class="provider">${esc(i.provider)}</div><h3>${esc(i.title)}</h3>${badges(i)}<div class="prize">🎁 ${esc(i.prize)}</div>${reasonBox(i)}<div class="mini-actions"><a class="primary" href="${esc(i.url)}" target="_blank" rel="noopener" onclick="registerClick('${esc(i.id)}')">Teilnehmen</a><button class="secondary" onclick="toggleFavorite('${esc(i.id)}')">${s.favorite?'♥':'♡'}</button></div></article>`}
function full(i){const s=stateFor(i.id),left=daysLeft(i),wins=i.winners?`${i.winners} bekannte Gewinne`:'Gewinnerzahl nicht angegeben';return `<article class="contest-card"><div class="card-top"><div><div class="provider">${esc(i.provider)}</div><h3>${esc(i.title)}</h3></div><button class="heart ${s.favorite?'active':''}" onclick="toggleFavorite('${esc(i.id)}')">${s.favorite?'♥':'♡'}</button></div>${badges(i)}<div class="prize">🎁 ${esc(i.prize)}</div><div class="scoreline"><strong>${label(i.score)}</strong><div class="scorebar"><i style="width:${i.score}%"></i></div><strong>${i.score}</strong></div>${reasonBox(i)}<div class="details">Teilnahmeschluss: ${esc(i.deadline)} · ${left===0?'endet heute':`${left} Tag${left===1?'':'e'} übrig`}<br>${esc(wins)} · Aufwand: ${'●'.repeat(Math.min(5,i.effort||3))}${'○'.repeat(Math.max(0,5-(i.effort||3)))}<br>${esc(i.country)} · geprüft: ${esc(i.verified||'–')}</div><div class="card-actions"><a href="${esc(i.url)}" target="_blank" rel="noopener" onclick="registerClick('${esc(i.id)}')">Teilnehmen ↗</a><button class="${s.done?'done':''}" onclick="toggleDone('${esc(i.id)}')">${s.done?'✓ Erledigt':'Teilgenommen'}</button></div></article>`}
function empty(t){return `<div class="empty">${esc(t)}</div>`}

function renderMetrics(){
 const a=scored(),done=a.filter(i=>stateFor(i.id).done).length;
 const m=[['🎯',a.filter(recommended).length,'heute lohnenswert','recommended'],['⭐',a.filter(i=>i.score>=80).length,'Top-Chancen','top'],['⏳',a.filter(i=>daysLeft(i)<=7).length,'enden bald','ending'],['✓',done,'teilgenommen','statsView']];
...
function discoverItems(){
 ...
 let l=scored().filter(i=>matches(i,currentFilter));
...
function renderPersonal(){
 const a=scored(),fav=a.filter(i=>stateFor(i.id).favorite),done=a.filter(i=>stateFor(i.id).done);
 $('#favoriteList').innerHTML=fav.map(full).join('')||empty('Deine Favoriten erscheinen hier.');
 $('#doneList').innerHTML=done.map(full).join('')||empty('Hier erscheinen deine markierten Teilnahmen.');
 const avg=done.length?Math.round(done.reduce((s,i)=>s+i.score,0)/done.length):0;
 $('#statsHero').innerHTML=`<strong>${done.length}</strong><p>Teilnahmen in deinem Gewinnjahr</p>`;
 const stats=[['♡',fav.length,'Favoriten'],['⭐',avg,'Ø Score'],['↗',Object.values(user.clicks||{}).reduce((a,b)=>a+b,0),'Teilnahme-Links geöffnet'],['🏆',a.filter(i=>stateFor(i.id).won).length,'Gewinne']];
...
async function loadData(){let fallback=true;try{const r=await fetch(`./data/contests.json?v=${Date.now()}`,{cache:'no-store'});if(r.ok){const p=await r.json();if(Array.isArray(p.contests)&&p.contests.length){contests=p.contests;fallback=false}}}catch(e){console.warn(e)}renderAll();if(fallback)toast('Offline-Daten geladen')}
...
user.lastVisit=new Date().toISOString();saveUser();loadData();
